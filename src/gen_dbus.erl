...
init([Module, Args]) ->
    case erlang:apply(Module, init, [Args]) of
        ...
        ignore ->
            ignore;
        {ok, {ServiceName, Path, DBusConfig}, SubState} ->
            ...
            setup(DBusConfig, State)
    end.

setup(DBusConfig, State) ->
    DefaultFace =
        case lists:keysearch(interface, 1, DBusConfig) of
    ...
    State1 = State#state{default_iface = DefaultFace},
    ...
    ?debug("Node: ~p~n", [Node]),
    XmlBody = dbus_introspect:to_xml(Node),
    State2 = State1#state{default_iface = Iface, node = Node, xml_body = XmlBody},
    {ok, State2}.
...
do_method_call(Module, Member, #dbus_message{header = Header} = Message, Conn, Sub) ->
    From =
        case Header#dbus_header.flags band ?NO_REPLY_EXPECTED of
            0 ->
                make_ref();
            _ ->
                none
