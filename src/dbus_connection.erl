...
handle_info({received, <<"DATA ", _Line/binary>>}, waiting_initial, State) ->
...
handle_info({received, <<"REJECTED ", __/binary>>}, _StateName, State) ->
    lager:info("handle_info REJECTED disconnected"),
    {stop, {error, disconnected}, State};

handle_info({received, <<"OK ", Line/binary>>}, waiting_initial, 
	    #state{sock=Sock, waiting=[{Pid, Tag}]}=State) ->
...
handle_info({received, <<"OK ", Line/binary>>}, authenticated, 
	    #state{sock=Sock, waiting=[{Pid, Header}]}=State) ->
    lager:info("connection:handle_info OK Line = ~p", [Line]),
    Guid = strip_eol(Line),
    lager:debug("GUID ~p~n", [Guid]),    
    ok = dbus_transport:setopts(Sock, [binary, {packet, raw}]),
...
handle_info({received, <<"ERROR", __/binary>>}, _StateName, State) ->
...
waiting_initial(auth, {Pid, Tag}, State) ->
    lager:info("connection:waiting_initial auth"),
    {reply, {Pid, Tag}, waiting_initial, State#state{waiting=[{Pid, Tag}]}};
    
waiting_initial({call, Header}, {Pid, _Tag}, State) ->
