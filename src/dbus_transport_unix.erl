...
In order to handle messages comming in from the tcp connection, messages are sent
directly to the process that called connect. The following messages are sent:
...
to receive communications from the socket. The reason for doing this is unclear, as
it does not seem necessary in `m:dbus_transport_tcp`, which operates similarly.
...
- `BusOptions` — a property list containing either a `path` key value pair or an
